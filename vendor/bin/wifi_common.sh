#!/vendor/bin/sh
#
# Copyright (c) 2017 - 2024 Amazon.com, Inc. or its affiliates.  All rights reserved.
...
# In AVLS-U usecase, when TV is connected to VPN,
# bridged's upstream interface should be either wifi or ethernet,
# instead of the VPN's interface.
handle_vpn_iface()
{
    NET_ROUTE_FILE="/proc/net/route"
    NET_ROUTE=`$CAT "$NET_ROUTE_FILE" 2>/dev/null`
    net_interface=""

    if grep -q "eth0.*00000000" "$NET_ROUTE_FILE"; then
        ilog "VPN is set up on network interface eth0"
        net_interface="eth0"
    elif grep -q "wlan0.*00000000" "$NET_ROUTE_FILE"; then
        ilog "VPN is set up on network interface wlan0"
        net_interface="wlan0"
    fi

    # if there's no active upstream interface that matches wlan or eth,
    # keep the UIF as empty string
    if [[ -z $net_interface ]]; then
        elog "cannot find the interface VPN is setup on."
        elog "/proc/net/route: $NET_ROUTE"
    fi
...
get_upstream_iface()
{
    ...
    # 1.1.1.1 via x.x.x.x dev UIF table x src x.x.x.x uid x
    UIF=${inet_array[4]}

    if ! [[ ($UIF == "eth"* || $UIF == "wlan"*) ]]; then
        ilog "NOTE: get the network interface VPN($UIF) is set up on."
        handle_vpn_iface
    fi
}
