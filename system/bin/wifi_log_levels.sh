...
function log_logcat
{
	...
	if [ $CHIPNSS = "2" ]; then
		logStr="$LOGNAME:rssi=$dataRssi;rssi2=$dataRssi2;noise=$maxNoise;noise2=$maxNoise2;channel=$CHANNEL;"
	else
		logStr="$LOGNAME:rssi=$dataRssi;noise=$maxNoise;channel=$CHANNEL;"
	fi
...
function clear_stale_stats
{
	...
	$IWPRIV $WLAN_INTERFACE driver "stat resetcounter" > /dev/null
}
